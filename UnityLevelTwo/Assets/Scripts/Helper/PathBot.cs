...
public sealed class PathBot : MonoBehaviour
{
    ...
    {
        //пробегаемся по всему Transform
        //Where собираем объекты, которые не равны нам
        //Select выбираем позиции только для наших объектов
        _nodes = GetComponentsInChildren<Transform>().
            Where(t => t != transform).Select(t => t.position).ToArray();
    }
